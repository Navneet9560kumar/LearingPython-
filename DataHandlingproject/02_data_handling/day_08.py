...
def falltten_json(data, parent_key='', sep='.'):
      items ={}
      ...
      if isinstance(data, dict):
            for k, v in data.items():
                  ...
                  print(full_key)
                  items.update(falltten_json(v,full_key, sep=sep))

      elif isinstance(data, list):
            for idx, item in enumerate(data):
                  full_key = f"{parent_key}{sep}{idx}" if parent_key else str(idx)
                  items.update(falltten_json(item, full_key, sep=sep))
      else:
...
def main():
      ...
      try:
            ...
            flattened = falltten_json(data, sep=sep)
